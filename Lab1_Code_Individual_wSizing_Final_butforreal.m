...
yline(0); % x axis
% labels
title('Lift Curve Slope');
ylabel('Coefficient of Lift (C_L)');
xlabel('Angle of Attack (\alpha) [degrees]');
legend('2-D Flat Plate','3-D Finite Wing','Location','southeast');
hold off
...
wFuselage = wConstant * (0.099*2 + 0.009);
wBallast = 0.4;
...
changeXH = 0.4;
wingChord = c;
changeXV = 0.25*sind(tailangle)*0.5+0.1;
...
ylabel('Wingspan [m]');
xlabel('Wing Loading');
title('Maximum Wingspan verses Wing Loading')
hold off

%% Coefficient of Moment

Cm_ac = 0.05; % guestimate

qinf = 0.5*rho*vMaxE^2;

%CL_ht = wTail/(qinf*Stail*cosd(tailangle));

xac = 0.25*c;

CL_w = sqrt(3*CD0/k1);

CL_ht = (Cm_ac + CL_w*((xcg-xac)/c))/VH;

tailangleSolve = acosd(0.295*9.81/(CL_ht*qinf));

AR_Tail = 2.5;

% lift curve slope equation (3-D) (eq. 1)
a = a0/(1+((57.3*a0)/(pi*e*AR_Tail)));

% 3-D coefficent of lift equation (eq. 2)
CL3D_Tail = a.*(AoA-AoA_L0);
...
alpha_e = 3.06; % guestimate
%i_t = 0; %incidence angle
...
alpha_w = 9.6; % guestimate

% incidence angle
i_t = alpha_w - alpha_e;

CM0 = Cm_ac + VH*AoA_L0*(i_t);
%AoA*((xcg-c/4)/c - alpha_e/AoA*VH(1-dEdA0A)
%dEdA0A experimental, 0.3-0.4
